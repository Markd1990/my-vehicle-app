    ...
    <div className="max-w-6xl mx-auto py-8 px-2 sm:px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          vehicles
            .filter(
              (v) => vehicleType === "All" || v.vehicle_type === vehicleType
            )
            .map((v) => (
              <Card
                key={v.id}
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-2">
                  {v.rental_details?.rental_image && (
                    <img
                      src={v.rental_details.rental_image}
                      alt="Rental"
                      className="object-cover h-8 w-8 rounded-full border-2 border-white shadow"
                    />
                  )}
                  {v.rental_details?.rental_name && (
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="text-muted-foreground text-sm">
                    Price/Day:{" "}
                    <span className="font-medium text-blue-950 text-xl">
                      ${v.price_perday}
                    </span>
                  </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-lg relative">
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-md h-32 flex items-center justify-center mb-2 bg-muted">
                {modal.vehicle_image ? (
                  <img
                    src={modal.vehicle_image}
                    alt={modal.vehicle_name}
                    className="object-contain h-28 w-full rounded"
                  />
                ) : (
                  <Car className="w-12 h-12 text-muted-foreground" />
              ...
              <div className="text-muted-foreground text-sm">
                Type: {modal.vehicle_type}
              </div>
              <div className="text-muted-foreground text-sm">
                Plate: {modal.plate_number}
              </div>
              <div className="text-muted-foreground text-sm">
                Price/Day:{" "}
                <span className="font-medium text-blue-950 text-xl">
              ...
              {modal.transmission && (
                <div className="text-muted-foreground text-sm">
                  Transmission: {modal.transmission}
                </div>
              )}
              {modal.fuel_type && (
                <div className="text-muted-foreground text-sm">
                  Fuel Type: {modal.fuel_type}
                </div>
              )}
              {modal.seats !== null && modal.seats !== undefined && (
                <div className="text-muted-foreground text-sm">
                  Seats: {modal.seats}
                </div>
              )}
              {modal.mileage !== null && modal.mileage !== undefined && (
                <div className="text-muted-foreground text-sm">
              ...
              {modal.airconditioned && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold">Feature:</span> Airconditioned
                </div>
              )}
              {modal.free_cancellation && (
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    disabled
                  >
                    <span className="sr-only">Free Cancellation</span>
                  </Button>
                  <span className="font-semibold">Feature:</span> Free
                  Cancellation
                </div>
              )}
            </div>
